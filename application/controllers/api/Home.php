...
class Home extends MY_Controller {
	...
	public function about_us() {
		try {
			$getAboutData = $this->db->query("SELECT manage_cms.id, manage_cms.title, manage_cms.description, banner.image FROM manage_cms JOIN banner ON manage_cms.title = banner.page_name WHERE manage_cms.id = 2 and manage_cms.status='Active' and banner.status= 'Active'")->result_array();
			if(!empty($getAboutData)) {
				$aboutData = array();
				foreach ($getAboutData as $abtKey => $abtValue) {
					$aboutData[$abtKey]['id'] = $abtValue['id'];
					$aboutData[$abtKey]['title'] = $abtValue['title'];
					$aboutData[$abtKey]['description'] = $abtValue['description'];
					$aboutData[$abtKey]['image'] = base_url().'uploads/banner/'.$abtValue['image'];
				}
			} else {
				$aboutData = 'No Data Found';
			}
			$response = array('status'=> 'success', 'result'=> $aboutData);
		} catch (\Exception $e) {
			$response = array('status'=> 'error', 'result'=> $e->getMessage());
		}
		echo json_encode($response);
	...
	public function privacy_policy() {
		try {
			$getPrivacyData = $this->db->query("SELECT manage_cms.id, manage_cms.title, manage_cms.description, banner.image FROM manage_cms JOIN banner ON manage_cms.title = banner.page_name WHERE manage_cms.id = 3 and manage_cms.status='Active' and banner.status= 'Active'")->result_array();
			if(!empty($getPrivacyData)) {
				$aboutData = array();
				foreach ($getPrivacyData as $abtKey => $abtValue) {
					$aboutData[$abtKey]['id'] = $abtValue['id'];
					$aboutData[$abtKey]['title'] = ucwords($abtValue['title']);
					$aboutData[$abtKey]['description'] = $abtValue['description'];
					$aboutData[$abtKey]['image'] = base_url().'uploads/banner/'.$abtValue['image'];
				}
			} else {
	...
	public function term_and_conditions() {
		try {
			$getPrivacyData = $this->db->query("SELECT manage_cms.id, manage_cms.title, manage_cms.description, banner.image FROM manage_cms JOIN banner ON manage_cms.title = banner.page_name WHERE manage_cms.id = 1 and manage_cms.status='Active' and banner.status= 'Active'")->result_array();
			if(!empty($getPrivacyData)) {
				$aboutData = array();
				foreach ($getPrivacyData as $abtKey => $abtValue) {
					$aboutData[$abtKey]['id'] = $abtValue['id'];
